...
async function UIorderDecision(ON, CONTRACT, COLLECTION) {
	...
	try {
		await orderDecision(ON, Decision, comments, CONTRACT, COLLECTION)
	} catch(err) {
		failureCallback(err);
	}
}

async function orderDecision(ON, DECISION, COMMENTS, CONTRACT, COLLECTION) {
	let transient = { orderID: ON, decision: DECISION, comments: COMMENTS, collection: COLLECTION};
	if (DECISION=='accept') {
...
async function makeParts(PN,ON,QTY,ChildLots,COLLECTION,CONTRACT){
	// submit transaction
...
async function UImakeparts(CONTRACT, COLLECTION) {
	...
	try {
		await makeParts(PN,ON,QTY,ChildLots,COLLECTION,CONTRACT);
	} catch (err) {
		failureCallback(err);
	}
}
...
// MODIFIED FOR API-AUTOSELECT: auto-determine available collection names
...
// create part and order
async function workflow(CONTRACT){
	let Part1 = 'E-1';
	let Part2 = 'E-1A';
	let Part3 = 'E-1B';
	let Order1 = 'O-1';
	let Order2 = 'O-1A';
	let Order3 = 'O-1B';
	let NAME = 'simulation part name';
	let PPN = '';
	let NumDims = 1;
	let DIMS = [1,10.1,1.5,3.7,1.33];
	let COLLECTION = 'OEM-S11collection';
	let DueDate = '2021.12.29';
	let QTY = 100;
	let SUPPLIER = 'OEM';
	try {
		await addDesignPart(Part1, NAME, PPN, NumDims, DIMS, CONTRACT, COLLECTION)
		await addDesignPart(Part2, NAME, Part1, NumDims, DIMS, CONTRACT, COLLECTION)
		await addOrder(Order1, Part1, DueDate, QTY, SUPPLIER, CONTRACT, COLLECTION)
		await addOrder(Order2, Part2, DueDate, QTY, 'S11', CONTRACT, COLLECTION)
		await addDesignPart(Part3, NAME, Part1, NumDims, DIMS, CONTRACT, 'OEM-S12collection')
		await addOrder(Order3, Part3, DueDate, QTY, 'S12', CONTRACT, 'OEM-S12collection')
	} catch (err) {
		console.log('Error detected:\n'.red + err)
		//return; //stop function here on error
	}
}

// accept order, make parts, inspect parts, ship parts
async function workflow2(CONTRACT){
	let PN = 'E-1'
	let COLLECTION = myCollection;
	let ON = 'O-10';
	let QTY = 100;
	let DECISION='accept';
	let COMMENTS='test run';
	try {  
		await orderDecision(ON, DECISION, COMMENTS, CONTRACT, COLLECTION)
	} catch (err) {
		failureCallback(err);
	...
	let ChildLots=[];
	let DimID = 1;
	let MEAS = [9.2,6.6,4.5,8.7]
	try {
		LN = await makeParts(PN,ON,QTY,ChildLots,COLLECTION,CONTRACT);
		await inspectParts(LN, PN, DimID, MEAS, CONTRACT, COLLECTION)
	} catch (err) {
		failureCallback(err);
		return; //stop function here on error
...
async function main() {
	try {
		...
		// start program timer (used to track total connection time for troubleshooting purposes)
		//console.time('programTimer');
		console.log("Starting App [=========100%=======]")
		try {
			//waitToContinue();
			await workflow(contractOrg1);
